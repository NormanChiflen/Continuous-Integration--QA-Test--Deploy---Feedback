#the end result is this:
  ...
  #  Hudson polls the VCS and ensures it has the latest copy of your codebase.
  #  If a change is detected in the codebase, it initiates a build using the above bash script.
  #  An .ipa is then created from the built app and the provisioning profile is embedded.
  #  A .zip file is created with the .ipa and the provisioning profile for legacy iOS3 users.
  #  The plist manifest file and the index.html web page is generated.
  #  All of these files plus the .dSYM directory are then copied across to your web server and versioned by the current Git hash.
